...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO tap_phim (id_phim, ten_tap, so_thu_tu, link_phim, mo_ta) 
            VALUES ($id_phim, '$ten_tap', $so_thu_tu, '$link_phim', '$mo_ta')";
    if (mysqli_query($conn, $sql)) {
        header("Location: quanlytap.php?id_phim=$id_phim");
        exit();
    } else {
        $thong_bao = "Lỗi thêm tập: " . mysqli_error($conn);
    }
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Thêm Tập Phim | FilmLane</title>
...
<body>
    <div class="form-container">
        <h2><i class="fas fa-plus-circle"></i> Thêm Tập Phim Mới</h2>
        <?php if($thong_bao != ""): ?>
        ...
        <form method="POST">
            ...
            <div style="display: flex; gap: 15px;">
                <div class="form-group" style="flex: 2;">
                    <label>Tên tập</label>
                    <input type="text" name="ten_tap" placeholder="VD: Tập 1: Bắt đầu" required>
                </div>
                <div class="form-group" style="flex: 1;">
                    <label>Số thứ tự</label>
                    <input type="number" name="so_thu_tu" value="1" min="1" required>
                </div>
            </div>

            <div class="form-group">
                <label>Link phim (URL hoặc mã nhúng)</label>
                <input type="text" name="link_phim" placeholder="https://..." required>
            </div>
            ...
            <button type="submit" class="btn-submit">THÊM TẬP PHIM</button>
